...
blockmed/.style ={rectangle, rounded corners, draw=black, fill=white,  text centered, text width=21em},
...
\node (investing_entrepreneurs) [blocklarge, below of=entrepreneurs, yshift=-3.5cm, xshift=-4.5cm] {\textbf{Investing Entrepreneurs} \\
...
$$\mathrm{s.t.} \quad c_{t}^i+\textcolor{red}{i_{t}^i}+q_{t}\left(n_{t+1}^i-\textcolor{red}{i_{t}^i}-\lambda n_{t}^i\right)+p_{t}\left(m_{t+1}^i-m_{t}^i\right)=r_{t} n_t^i$$
$$ n_{t+1}^i \geq \textcolor{red}{(1-\theta) i_{t}^i}+\left(1-\phi_{t}\right) \lambda n_{t}^i$$
$$m_{t+1}^i \geq 0$$
$$k_{t+1}^i=\lambda k_{t}^i+\textcolor{red}{i_{t}^i}$$ 
};

\node (noninvesting_entrepreneurs) [blockmed, below of=entrepreneurs, yshift=-3.5cm, xshift=4.5cm] {\textbf{Non-investing Entrepreneurs} \\
$$ \max_{c_t^s, n_{t+1}^s, m_{t+1}^s} E_{t} \sum_{s=t}^{\infty} \beta^{s-t} u\left(c_{s}^s\right)$$
$$\mathrm{s.t.} \quad c_{t}^s+q_{t}\left(n_{t+1}^s-\lambda n_{t}^s\right)+p_{t}\left(m_{t+1}^s-m_{t}^s\right)=r_{t} n_t^s$$
$$ n_{t+1}^s \geq \left(1-\phi_{t}\right) \lambda n_{t}^s$$
$$m_{t+1}^s \geq 0$$
$$k_{t+1}^s=\lambda k_{t}^s$$ 
};

\node (policy) [blockmed, below of=investing_entrepreneurs, yshift= -4cm] {\textbf{Monetary policy} \\
...
\path[every node/.style={font=\sffamily\small}]
    (entrepreneurs) edge node [right] {} (investing_entrepreneurs)
